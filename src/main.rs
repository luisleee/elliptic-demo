...
use elliptic::{Curve, GFp, Point, U256};
...
fn main() {
    println!("---===NIST P-256 demo===---");
    let (curve, g) = prepare_nist_p256();
    println!("Curve secp256r1:");
    println!("{}", curve);
    println!("Base Point G:");
    println!("{}", g);

    println!("Now we compute kG");
    let mut scalar_input = String::new();
    print!("Enter scalar k (hex): ");
    io::stdout().flush().unwrap();
    io::stdin().read_line(&mut scalar_input).unwrap();
    let scalar = match U256::from_hex_str(&scalar_input) {
        Ok(val) => val,
        Err(e) => {
            eprintln!("Error parsing scalar: {}", e);
            return;
        }
    };
